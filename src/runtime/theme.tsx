...
function PostView({ page, pages, config }: { page: Page; pages: Page[]; config: DavipressConfig }) {
  ...
  return <PostChrome headings={page.headings} title={String(page.frontmatter.title ?? '')} hasComments={commentsEnabled} route={page.route}>
    <div className="dp-post">
      <article className="dp-post-article markdown-body">
        ...
        {updated && <div className="dp-post-update"><div><MdHistory aria-hidden="true" /><span>Cập nhật: {formatDate(updated)}</span></div></div>}
        {(previous || next) && <div className="dp-post-nav"><div>{previous && <a href={previous.route}>← {String(previous.frontmatter.title)}</a>}</div><div>{next && <a href={next.route}>{String(next.frontmatter.title)} →</a>}</div></div>}
...
function TutorialView({ page, config }: { page: Page; config: DavipressConfig }) {
  const contentHtml = page.html.replace(/^\s*<h1\b[^>]*>[\s\S]*?<\/h1>\s*/, '')
  const readingTime = Math.max(1, Math.ceil(page.text.trim().split(/\s+/).filter(Boolean).length / 200))
  const commentsEnabled = Boolean(config.giscus?.enabled && page.frontmatter.comments !== false)

  return <article className="markdown-body dp-tutorial-article">
    <h1>{page.frontmatter.title}</h1>
    {Boolean(page.frontmatter.subtitle) && <p className="dp-tutorial-subtitle">{String(page.frontmatter.subtitle)}</p>}
    <div className="dp-tutorial-meta">
...
export async function DocsTheme({ page, config }: { page: Page; config: DavipressConfig }) {
  ...
  return <div className="davipress-shell">
    <DocsChrome sidebar={sidebar} headings={page.headings} activeRoute={page.route} title={String(page.frontmatter.title ?? page.headings[0]?.text ?? '')} hasComments={hasComments} footer={<Footer footer={config.themeConfig?.footer} />} reveal={isTutorial}>
      {page.route === '/' && <div className="dp-home-hero"><div><h1>{page.frontmatter.title}</h1><p>{page.frontmatter.description}</p></div><AvatarStack /></div>}
      {isTutorial ? <TutorialView page={page} config={config} /> : <article className={`markdown-body ${page.route === '/' ? 'dp-home-content' : ''}`} dangerouslySetInnerHTML={{ __html: page.html }} />}
    </DocsChrome>
    <NavBar items={nav} />
  </div>
}