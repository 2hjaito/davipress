...
function generate() {
  if (!fs.existsSync(path.join(cwd, 'package.json'))) throw new Error('Davipress: package.json is required. Run npm init first.')
  ...
  const pkg = JSON.parse(fs.readFileSync(path.join(cwd, 'package.json'), 'utf8')) as { type?: string; scripts?: Record<string, string> }
  pkg.type = 'module'
  pkg.scripts ??= {}
