...
function groups(content: string, split: string) {
  return content
...
function extractUsernameFromConfig(config?: DavipressConfig): string {
...
async function fetchGithubRepos(username: string, topic?: string): Promise<ProjectItem[]> {
  try {
    ...
    return rawList.map(repo => {
      ...
      const topics = rawTopics.filter(
        (t: string) => t.toLowerCase() !== repo.language?.toLowerCase()
      )
...
function sortProjects(items: ProjectItem[], sort = 'updated-desc'): ProjectItem[] {
...
export async function loadProjects(
...
): Promise<{ title: string; blocks: ProjectBlock[] }> {
  let sourceContent = ''
  let title = 'Dự án'
  ...
  for (const match of matches) {
    ...
    if (type === 'projects' || type === 'github-repositories') {
      ...
      let items: ProjectItem[] = manualGroups
        .map(g => ({
          name: value(g, 'name'),
      ...
      const items: ToolItem[] = toolGroups
        .map(g => ({
          title: value(g, 'title'),
