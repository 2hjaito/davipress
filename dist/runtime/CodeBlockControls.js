...
export function CodeBlockControls() {
    useEffect(() => {
        let overlay = null;
        const frameFor = (pre) => pre.closest('.dp-code-frame');
        const closeZoom = () => {
            document.querySelectorAll('.dp-code-frame.dp-code-zoomed').forEach(frame => frame.classList.remove('dp-code-zoomed'));
            overlay?.remove();
            overlay = null;
        };
        const toggleCollapsed = (pre) => {
            const frame = frameFor(pre);
            if (!frame)
                return;
            if (frame.classList.toggle('dp-code-collapsed'))
                closeZoom();
        };
        const addControls = () => {
            document.querySelectorAll('.markdown-body pre').forEach(pre => {
                const existingFrame = frameFor(pre);
                if (existingFrame?.querySelector('.dp-copy-code'))
                    return;
                ...
                [['dp-code-close', 'x'], ['dp-code-minimize', '-'], ['dp-code-zoom', '+']].forEach(([className, label]) => {
                    ...
                    button.onclick = () => {
                        if (label === '+') {
                            if (frame.classList.contains('dp-code-zoomed'))
                                return closeZoom();
                            closeZoom();
