...
export function HomeView({ blocks, footer }) {
    ...
    return _jsxs("div", { className: "dp-home-view", children: [blocks.map((block, index) => {
                ...
                if (block.type === 'certifications') {
                    ...
                    return _jsx(CertGroups, { title: certifications.title, groups: [{ org: 'Certifications', logo: '', certifications: certifications.items.map(item => ({ title: item.title, image: item.img, level: formatDate(item.date) })) }] }, index);
                }
                return null;
