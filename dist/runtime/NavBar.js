...
export function NavBar({ items = defaultItems }) {
    ...
    return _jsx("div", { className: "dp-navbar", children: _jsxs("div", { className: "dp-navbar-items", children: [items.map(item => { const label = 'text' in item ? item.text : item[0]; const href = 'text' in item ? item.link : item[1]; const Icon = iconMap[label] ?? GiSpellBook; return _jsx("a", { href: href, title: label, onClick: () => setActive(href), className: "dp-nav-item", children: active === href ? _jsx(FaSpinner, { className: "dp-spin", "aria-hidden": "true" }) : _jsx(Icon, { "aria-hidden": "true" }) }, href); }), _jsx("button", { type: "button", onClick: toggle, title: "Toggle theme", className: "dp-nav-item", children: dark ? _jsx(FaMoon, { "aria-hidden": "true" }) : _jsx(FaSun, { "aria-hidden": "true" }) })] }) });
}
