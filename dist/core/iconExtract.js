    ...
    if (stats.isDirectory()) {
        for (const entry of fs.readdirSync(target))
            if (entry !== 'node_modules' && !entry.startsWith('.'))
                scan(path.join(target, entry), found);
        return;
    }
    if (!/\.(md|mdx|ts|tsx|js|jsx|json|ya?ml)$/.test(target))
        return;
    for (const line of fs.readFileSync(target, 'utf8').split('\n')) {
        if (!/\bicons?\b\s*[:=]/.test(line))
            continue;
        for (const match of line.matchAll(iconNamePattern))
            if (packs.has(match[1].slice(0, 2).toLowerCase()))
                found.add(match[1]);
    }
}
/** Icon names referenced from a site's config and content, e.g. `icon: DiPhotoshop`. */
export function findIconNames(cwd) {
    const found = new Set();
    for (const target of ['davipress.config.ts', 'config', 'docs', 'src/config'])
        scan(path.join(cwd, target), found);
    return [...found].sort();
}
/** Reads icon definitions straight out of the davi-icons pack sources so only the used ones get bundled. */
export function extractIcons(names, packsDir) {
    ...
    for (const name of names) {
        ...
        for (let cursor = open; cursor < source.length; cursor++) {
            if (source[cursor] === '{')
                ...
                const icon = JSON.parse(source.slice(open, cursor + 1));
